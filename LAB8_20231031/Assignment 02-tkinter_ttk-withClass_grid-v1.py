...
class Main(Tk):
    ...
    def __init__(self,):
        ...
        self.optionFrame = self.optionFrame(self.mainFrame, x=0, y=0, width=1, height=1)

    def buildFrame(self, host, x, y, width, height, pos=(N,W,E,S)):
        res = ttk.Labelframe(host, padding=5)
        res.grid(column=x, row=y, columnspan=width, rowspan=height, sticky=pos)
        return res

    def buildLabelFrame(self, host, text, x, y, width, height, pos=(N,W,E,S)):
